...
import { getTestHistories, getVocabularyCount } from '../api/user/route';
import { formatDate } from '@/utils/formatDate';

export default async function Test() {
	const testHistories = await getTestHistories();
	const vocabularyCount = await getVocabularyCount();

	return (
		<>
			<PageTitle title="テスト" />
			<TestStartModal vocabularyCount={vocabularyCount} />
			{vocabularyCount >= 10 && (
				<Box>
					<Typography
						variant="h4"
